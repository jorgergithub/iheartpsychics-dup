...
namespace :calls do
  ...
  task :process => :environment do
    account = TwilioHelper.client.account
    ClientCall.unprocessed.each do |call|
      twilio_call = account.calls.get(call.sid)
      ...
      attributes = %w(parent_call_sid date_created date_updated account_sid
                      to from phone_number_sid status start_time end_time duration
                      price price_unit direction answered_by caller_name uri)

      # attributes.each { |a| puts "#{a} = [#{twilio_call.send(a).inspect}]" }
